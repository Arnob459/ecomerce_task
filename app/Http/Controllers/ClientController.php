...
class ClientController extends Controller
{
    ...
    public function AddProductToCart(Request $request){
        ...
        $request->validate([
            'product_id' => 'required|integer|max:50',
            'quantity' => 'nullable|integer|max:60',
        ]);
        ...
        if ($product) {
            ...
            return redirect()->route('user.addtocart')->with('message', 'Your item added to cart successfully!');
        } else {
            return back()->with('error', 'Please choose valid product');
    ...
    public function RemoveCartItem($id){
        ...
        return redirect()->route('user.addtocart')->with('message', 'Your item removed form cart successfully!');
    }
    ...
    public function Checkout(){
        $userid = Auth::id();
        $cart_items = Cart::where('user_id', $userid)->with('product')->get();
